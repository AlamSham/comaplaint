...
const filePath = path.join(__dirname, 'seedTemplates.ts');
let content = fs.readFileSync(filePath, 'utf-8');

// The new templates to add
...
function generateMetadata(title, language) {
  const langLabel = language === 'hindi' ? 'Hindi' : language === 'hinglish' ? 'Hinglish' : 'English';
  
  // Basic SEO title focused on "Format", "Draft", "PDF"
  let seoTitle = title + ' Format in ' + langLabel + ' | Free Draft PDF';
  if (seoTitle.length > 60) {
    seoTitle = title + ' Format | Draft PDF';
  }
  
  const description = 'Download free ' + title.toLowerCase() + ' format in ' + langLabel + '. Copy-paste ready draft for consumer court, legal notices, and official complaints. PDF download available.';

  return '\\n    metadata: {\\n      title: \\'' + seoTitle + '\\',\\n      description: \\'' + description + '\\',\\n    }';
}

// Transform the content by finding the closing brace of each template and inserting metadata if not present
let updatedContent = content.replace(/content:\s*\`[\s\S]*?\`/g, (match) => {
  // We need to inject metadata right after content
  return match;
});

// Let's do it by splitting on "}," and ensuring we process correctly,
// Actually, a simpler way is to match `language: '(.*?)'` and `title: '(.*?)'`
// Since it's a JS file, let's use eval to get the array, modify it, and write it back.
// But it has TypeScript exports.
const arrayStrMatch = content.match(/export const templatesData = (\[[\s\S]*?\]);/);
if (arrayStrMatch) {
  // Not using eval because of template literals inside with backticks.
  // Instead, let's just do a regex replace to insert metadata before the closing brace of each object
  let newContent = content;
  
  const matches = [...content.matchAll(/title:\s*'([^']*)',\s*language:\s*'([^']*)',[\s\S]*?content:\s*(\`[\s\S]*?\`)\s*,?\s*}/g)];
  ...
  for (const match of matches) {
    ...
    if (fullMatch.includes('metadata:')) continue; // Skip if already has metadata
    
    const metadataStr = generateMetadata(title, language);
    
    const replacedMatch = fullMatch.replace(/}\s*$/, \`,\${metadataStr}\n  }\`);
    newContent = newContent.replace(fullMatch, replacedMatch);
  }

  // Now append the new templates before the closing bracket ];
  newContent = newContent.replace(/];/, newTemplatesStr.trim());
  
  fs.writeFileSync(filePath, newContent, 'utf-8');
  console.log('Successfully updated seedTemplates.ts');
} else {
  console.error('Could not parse templatesData array');
}
